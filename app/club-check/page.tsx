...
export default function ClubCheckPage() {
  ...
  return (
    <SiteShell title="Tribunetour · Klubtjek">
      ...
      <section className="site-card-soft p-5 md:p-7"><div className="label-eyebrow">Kontrolhistorik</div><h2 className="mt-2 text-xl font-semibold">Seneste registreringer</h2><div className="mt-4 grid gap-2 text-sm text-[var(--muted)]">{history.slice(0, 8).map((entry) => { const complete = (!entry.clubId.startsWith('dk-') || entry.checks.fixtures) && entry.checks.location && entry.checks.league; return <div key={`${entry.timestamp}-${entry.clubId}`} className="flex flex-col justify-between gap-1 border-b border-white/10 py-3 sm:flex-row"><span><strong className="text-[var(--text)]">{entry.club}</strong> · {complete ? 'godkendt' : 'delvist kontrolleret'}</span><span>{formatKickoff(entry.timestamp)}</span></div>; })}{!history.length && <p>Ingen kontroller er gemt endnu.</p>}</div></section>
    </SiteShell>
  );
}
